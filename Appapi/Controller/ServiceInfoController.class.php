...
class ServiceInfoController extends AppapiController {
    public function index()
	{
		...
		{			
			...
			case 6: //add / update service type
				...
				if(I('get.holdId')) $data['hold_id']=I('get.holdId');
				if(I('get.agedId')) $data['aged_id']=I('get.agedId');
				if(I('get.userId')) $data['user_id']=I('get.userId');
			...
			case 3: //add / update service info
				$model = M('Pservice_info');
				unset($data);
				$data['id']=(int)I('param.serviceId');
				//if(I('param.holdId')) $data['holdId']=I('param.holdId');
				//if(I('param.agedId')) $data['agedId']=I('param.agedId');
				$data['loc_id']=(int)I('param.holdId');
				$data['service_name']=I('param.name');
				$data['type_id']=(int)I('param.type_id');
				//$data['imgbyte']=I('param.imgByte');
				if(I('param.imgByte')){
					//解码
					...
					if(!$data['id']) $data['photo_src']='';
				}
				$data['business']='';
				$data['star_level']=(int)I('param.star');
				$data['phone']=I('param.tel');
				$data['address']=I('param.address');
				$data['remark']=I('param.remark');
				$data['is_private']=(int)I('param.isPrivate');
				if($data['id']){
					$setting_flg = $model->data($data)->save();				
				}else{
					$data['status_flg']='Y';
					$data['create_dt']=getCurrTime();
					$data['service_code']='S'.getCurrTime();
					$setting_flg = $model->data($data)->add();
					//var_dump($setting_flg);var_dump($data);exit;
					unset($data);
					$data['service_id']=(int)$setting_flg;
					$data['elderly_id']=(int)I('param.agedId');
